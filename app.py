    ...
    if st.button("Extract Data", type="primary"):
        ...
        try:
            ...
            - FOR "Designation": Construct a string following EXACTLY this format: [Capacity] [Tolerance] [Voltage] [Description] [Package Type EIA]. 
              * Example: "0.10UF 20% 100V X7T 1210". 
              * CRITICAL FOR CAPACITY: In the Designation string, use the exact capacity value and unit extracted, but ensure microfarads are represented as "UF" or "µF" (e.g., "0.10UF", "10UF"), and NEVER use "MF".
              * CRITICAL FOR TOLERANCE: For [Tolerance] in the Designation string, you MUST use the numeric percentage (e.g., 5%, 20%) and NEVER the letter code (e.g., J, K, M). Remove any '±' symbol. Include the % and V symbols.
              * CRITICAL FOR PACKAGE TYPE: For [Package Type EIA] inside this designation string, use ONLY the bare numeric code (e.g., 0603, 1206). Do NOT include the "EIA" prefix or the "*" asterisk.
              * CRITICAL FOR DESCRIPTION: When inserting the [Description] into the Designation string, strictly remove any category prefixes (like "KERAMIK-", "SILICON-", or "FOLIE-"). For example, if the description is "KERAMIK-X7R", use ONLY "X7R" in this designation string.
            ...
            - FOR "Height (Max)": Extract the MAXIMUM thickness/height value. If it states a nominal value with tolerance (e.g., "1.4mm +/-0.15mm" or "2.5 ± 0.30 mm"), calculate the maximum value precisely by adding the nominal value and the positive tolerance (e.g., 1.4 + 0.15 = 1.55). If it explicitly states a max value (e.g., "2.8mm MAX"), extract that value directly.
